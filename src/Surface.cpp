...
void Surface::dDivXsurface() {
	...
	for(int i = 0; i < length; ++i) {
		setDDivx(0,i,(access2D(2,i,vertecies).y + access2D(1,i,vertecies).y -
			2*access2D(0,i,vertecies).y)/ddist);
	}
	//width edge (at width -1)
	for(int i = 0; i<length; ++i) {
		setDDivx(width-1,i,(access2D(width-3,i,vertecies).y + access2D(width-2,i,vertecies).y -
			2*access2D(width-1,i,vertecies).y)/(ddist));
	}

	for(int i = 0; i < length; ++i)
		for(int j = 1; j < width-1; ++j) {
			setDDivx(j,i,(access2D(j-1,i,vertecies).y + access2D(j+1,i,vertecies).y -
				2*access2D(j,i,vertecies).y)/ddist);
	}
}
...
void Surface::dDivYsurface() {
	...
	for(int i = 0; i < width; ++i) {
		setDDivy(i,0,(access2D(i,2,vertecies).y + access2D(i,1,vertecies).y -
			2*access2D(i,0,vertecies).y)/ddist);
	}

	for(int i = 0; i<width; ++i) {
		setDDivy(i,length-1,(access2D(i,length-2,vertecies).y + access2D(i,length-3,vertecies).y -
			2*access2D(i,length-1,vertecies).y )/ddist); //backwards
	}

	for(int i = 0; i < width; ++i)
		for(int j = 1; j < length-1; ++j) {
			setDDivy(i,j,(access2D(i,j-1,vertecies).y + access2D(i,j+1,vertecies).y -
				2*access2D(i,j,vertecies).y)/ddist);
	}
}
...
void Surface::calculateSurfaceHeight() {
	...
	for(int i = 0; i< length*width; ++i) {
		...
		vertecies[i].y = vertecies[i].y*DAMP + time*plane[i].div_h;
	}
}
...
void Surface::rainDrop(int x, int y) {
	setDivh(x,y,accessVert(x,y).div_h -0.5f);
}
